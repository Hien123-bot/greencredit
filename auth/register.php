...
$error = "";
if (isset($_GET['error'])) {
    if ($_GET['error'] == 'email_exists') $error = "Email này đã được sử dụng.";
    else if ($_GET['error'] == 'empty') $error = "Vui lòng nhập đầy đủ thông tin.";
    else if ($_GET['error'] == 'password_mismatch') $error = "Mật khẩu không khớp.";
}
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <title>Đăng ký | Green Credit</title>
    ...
    <style>
        ...
        .mesh-gradient {
            background: radial-gradient(at 100% 0%, #f0fdf4 0px, transparent 50%),
                        radial-gradient(at 0% 100%, #f8fafc 0px, transparent 50%);
            background-color: #ffffff;
        }
    </style>
</head>
<body class="mesh-gradient min-h-screen flex items-center justify-center p-6 py-12">
    ...
    <div class="w-full max-w-lg" data-aos="fade-up">
        ...
        <div class="flex flex-col items-center mb-6">
            ...
            <h2 class="text-xs font-bold text-green-600 uppercase tracking-widest">Đăng ký tham gia</h2>
        </div>

        <!-- Compact Card -->
        <div class="compact-card p-8 md:p-10">
            <header class="mb-8 text-center">
                <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Tạo tài khoản</h1>
            </header>

            <?php if (isset($_GET['msg']) && $_GET['msg'] == 'scan_success'): ?>
                <div class="bg-green-50 text-green-700 px-4 py-3 rounded-xl mb-6 text-xs font-semibold flex items-center gap-2 border border-green-100" data-aos="zoom-in">
                    <span class="material-symbols-outlined text-[18px]">qr_code_2</span>
                    Đã nhận mã QR! Đăng ký ngay để nhận điểm.
                </div>
            <?php endif; ?>
            ...
            <form action="register_process.php" method="POST" class="space-y-5">
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5 ml-1">Họ và tên</label>
                    <input type="text" name="full_name" required placeholder="Nguyễn Văn A" 
                           class="w-full px-5 py-3 rounded-xl font-medium text-slate-900 text-sm bg-slate-50 border border-slate-200 focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all outline-none">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 ml-1">Số điện thoại</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 ml-1">Mật khẩu</label>
                        <input type="password" name="password" required placeholder="••••••••" 
                               class="w-full px-5 py-3 rounded-xl font-medium text-slate-900 text-sm bg-slate-50 border border-slate-200 focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 ml-1">Xác nhận</label>
            ...
            <div class="mt-8 pt-6 border-t border-slate-100 text-center text-sm font-medium text-slate-500">
                Đã có tài khoản? <a href="login.php" class="text-green-600 font-semibold hover:underline ml-1">Đăng nhập</a>
            </div>
        </div>
